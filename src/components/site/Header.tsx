...
import { Search, Heart, ShoppingBag, Menu, X, Sun, Moon, Minus, Plus, Trash2 } from "lucide-react";
import { categories, inr, products } from "@/data/products";
import { useShop } from "@/context/shop";

export function Header() {
  const { cart, cartCount, subtotal, removeLine, setQty, theme, toggleTheme, wishlist } = useShop();
  ...
  return (
    <>
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-[auto_1fr_auto] items-center gap-3 px-4 py-3 sm:px-6 lg:grid-cols-3">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <nav className="hidden items-center gap-6 text-xs uppercase tracking-[0.16em] lg:flex">
              <Link to="/shop" search={{ group: "Western Wear" }} className="link-underline">
                Western
              </Link>
              <Link to="/shop" search={{ group: "Indian Wear" }} className="link-underline">
                Indian
              </Link>
          ...
          <Link to="/" className="justify-self-center text-center">
            <span className="text-xl font-semibold tracking-[0.42em] uppercase sm:text-2xl">
              Eagon
            </span>
      ...
      {menu && (
        <div className="fixed inset-0 z-60 bg-background/95 p-6 backdrop-blur-xl lg:hidden">
        ...
        <div className="fixed inset-0 z-60 flex justify-end bg-foreground/30 backdrop-blur-sm">
          ...
          <aside className="flex h-full w-full max-w-sm flex-col bg-card p-6 shadow-soft">
            ...
            <div className="mt-6 border-t border-border pt-4">
              ...
              <button
                type="button"
                className="mt-4 w-full bg-primary py-3 text-xs font-semibold uppercase tracking-[0.2em] text-primary-foreground"
              >
                Secure checkout
              </button>
